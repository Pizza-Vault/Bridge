...
@router.post("/create", dependencies=[Depends(auth_guard_factory("POST", "/api/order/create"))])
async def create_order(body: dict, idem_key: str | None = Header(None, alias="Idempotency-Key"), db: AsyncSession = Depends(_db)):
...
@router.get("/status", dependencies=[Depends(auth_guard_factory("GET", "/api/order/status"))])
...
@router.post("/complete", dependencies=[Depends(auth_guard_factory("POST", "/api/order/complete"))])
